...
BOTS = {
    "Googlebot": {
        ...
        "description": "Google web crawling",
    },
    "Bingbot": {
        "category": "Search Engines",
        "user_agent": "bingbot",
        "description": "Bing web crawling",
    },
    "Slurp": {
        "category": "Search Engines",
        "user_agent": "Slurp",
    ...
    "GPTBot": {
        "category": "A.I. Bots",
        "user_agent": "GPTBot",
        "description": "OpenAI training data",
    },
    "ChatGPT-User": {
        "category": "A.I. Bots",
        "user_agent": "ChatGPT-User",
        "description": "OpenAI real-time browsing",
    },
    "OAI-SearchBot": {
        "category": "A.I. Bots",
        "user_agent": "OAI-SearchBot",
        "description": "OpenAI search indexing",
    },
    "anthropic-ai": {
        "category": "A.I. Bots",
        "user_agent": "anthropic-ai",
        "description": "Anthropic training",
    },
    "ClaudeBot": {
        "category": "A.I. Bots",
        "user_agent": "ClaudeBot",
        "description": "Anthropic Claude browsing",
    },
    "CCBot": {
        "category": "A.I. Bots",
        "user_agent": "CCBot",
        "description": "Common Crawl (AI datasets)",
    },
    "cohere-ai": {
        "category": "A.I. Bots",
...
def fetch(url: str, ua: str = BROWSER_UA, timeout: int = 8):
...
def _parse_robots_for_agent(raw: str, path: str, user_agent: str):
    ...
    disallow_paths = [
        p or "/" for t, p in sec["rules"] if t == "disallow" and path_matches(p, path)
    ]
...
def _directives(obj) -> str:
    labels = []
    if getattr(obj, "none_directive", False):
        labels.append("none")
    if getattr(obj, "noindex", False):
        labels.append("noindex")
    if getattr(obj, "nofollow", False):
        labels.append("nofollow")
    if getattr(obj, "noarchive", False):
        labels.append("noarchive")
    if getattr(obj, "nosnippet", False):
        labels.append("nosnippet")
    if getattr(obj, "noimageindex", False):
        labels.append("noimageindex")
...
def print_result(r: CheckResult):
    ...
    # robots.txt
    rs = (
        f"{GREEN}Fetched  HTTP {r.robots_txt_status}{RESET}"
    ...
    for cat, bots in categories.items():
        ...
        for br in bots:
            ...
            delay = (
                f"  {DIM}crawl-delay: {br.crawl_delay}s{RESET}"
                if br.crawl_delay
                else ""
            )
            print(
                f"   {_ok(br.allowed)}  {BOLD}{br.name}{RESET} "
    ...
    if mr.present:
        ...
        print(
            f'   {colour}PRESENT{RESET}  <meta name="robots" content="{mr.raw_value}">'
        )
        print(f"   Directives : {_directives(mr)}")
        if mr.noindex or mr.none_directive:
            print(
                f"   {RED}⚠  noindex — search engines must not index this page{RESET}"
            )
        if mr.nofollow:
            print(
                f"   {YELLOW}⚠  nofollow — links on this page should not be followed{RESET}"
            )
    else:
    ...
    if cn.present:
        self_lbl = (
            f"{GREEN}self-referencing ✔{RESET}"
    ...
    print(
        f"   Indexable     : {icolour}{'Yes' if indexable else 'No  (noindex is set)'}{RESET}"
    )
...
def read_urls_from_excel(file_path: str) -> list:
    """Read URLs from Excel file (first column with 'link', 'url', or just first column)"""
    try:
        ...
        # Find the link/url column
        url_col_index = None
        for i, header in enumerate(headers):
            if header and isinstance(header, str):
                if "link" in header.lower() or "url" in header.lower():
                    url_col_index = i
                    break

        # If no link/url column found, use first column
        if url_col_index is None:
            url_col_index = 0

        # Read URLs from the column
        for row_idx, row in enumerate(
            ws.iter_rows(min_row=2, values_only=False), start=2
        ):
            cell_value = row[url_col_index].value
...
def write_results_to_excel(results: list, output_file: str):
    """Write check results to Excel file"""
    try:
        wb = Workbook()
        ws = wb.active
        ws.title = "Results"
        ...
        for result in results:
            total_bots = len(result.bot_results)
            blocked_bots = sum(1 for b in result.bot_results if not b.allowed)
            indexable = not (
                result.meta_robots.noindex
                or result.meta_robots.none_directive
                or result.x_robots_tag.noindex
                or result.x_robots_tag.none_directive
            )

            row = [
                result.url,
                "Yes" if result.robots_txt_fetched else "No",
                total_bots,
                blocked_bots,
                "Yes" if indexable else "No",
                "Yes" if result.meta_robots.present else "No",
                "Yes" if result.x_robots_tag.present else "No",
                "Yes" if result.canonical.present else "No",
            ]
            ws.append(row)
...
def main():
    ...
    if args and args[0].endswith(".xlsx"):
        input_file = args[0]
        output_file = args[1] if len(args) > 1 else "results.xlsx"
        ...
        # Use ThreadPoolExecutor for parallel processing (8 workers)
        max_workers = min(8, len(urls))
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all URLs for processing
            future_to_url = {
                executor.submit(check_url, url): (i + 1, url)
            ...
            for future in as_completed(future_to_url):
                ...
                try:
                    ...
                    status = "✓" if result.robots_txt_fetched else "✗"
                    blocked = sum(1 for b in result.bot_results if not b.allowed)
                    print(
                        f"[{completed}/{len(urls)}] {status} {url} ({blocked} bots blocked)"
                    )
                except Exception as e:
                    print(f"[{completed}/{len(urls)}] ✗ {url} (Error: {str(e)[:50]})")
        ...
        # Original behavior: check URLs from command line
        urls = args
        if not urls:
            print("Usage: python robots_excel.py <url> [url2 ...]")
            print("   or: python robots_excel.py input.xlsx [output.xlsx]")
            print("Example: python robots_excel.py https://www.example.com/")
            sys.exit(1)
...
# python robots_excel.py your_links.xlsx
# # or specify output name:
# python robots_excel.py your_links.xlsx results.xlsx
