...
function Lightbox({
  ...
  return (
    <AnimatePresence>
      {open && (
        <motion.div
          className="fixed inset-0 bg-black/90 z-[60] flex items-start justify-center pt-[5vh] px-[5vw] pb-[5vh]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <img
                ...
                alt=""
                className="w-full h-full object-contain select-none p-4 max-h-[85vh]"
                draggable={false}
              />
            </motion.div>
          </AnimatePresence>
